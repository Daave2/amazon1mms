...
def test_build_batch_chat_payload_includes_attention_summary():
    ...
    assert "2" == card["sections"][0]["widgets"][3]["decoratedText"]["text"]
    assert "Belle Vale: Lates 4.5 %, INF 2.1 %" in card["sections"][2]["widgets"][0]["textParagraph"]["text"]
    assert "Welling: UPH 52" in card["sections"][2]["widgets"][0]["textParagraph"]["text"]
