...
from core.config import LOCAL_TIMEZONE, FORM_POST_URL, FIELD_MAP, LOG_FILE, JSON_LOG_FILE, DEBUG_MODE
            ...
            try:
                ...
                async with session.post(FORM_POST_URL, data=payload, timeout=10) as resp:
                    if resp.status == 200:
                        ...
                        await state.add_failure(f"{store_name} (HTTP Submit Fail {resp.status})", asyncio.get_event_loop().time())
