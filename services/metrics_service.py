...
from core.config import (
    BASE_DASHBOARD_URL, PAGE_TIMEOUT, WAIT_TIMEOUT, WORKER_RETRY_COUNT,
    SPECIAL_NAME_MAPPINGS, LOCAL_TIMEZONE
)
...
async def process_single_store(page: Page, store_info: Dict[str,str], submission_queue: asyncio.Queue, state: ScraperState):
    ...
    for attempt in range(WORKER_RETRY_COUNT):
        try:
            ...
            if not api_data:
                ...
                if ("summationMetrics" in req_url or "api/metrics" in req_url) and not state.cache.api_url_template:
                    async with state.cache.lock:
                        ...
                        app_logger.info(f"[{store_name}] Discovery: Captured API Template: {state.cache.api_url_template[:100]}...")
            ...
            await state.record_metric(store_name, duration, int(data_to_use.get('OrdersShopped_V2', 0)), int(data_to_use.get('RequestedQuantity_V2', 0)))
