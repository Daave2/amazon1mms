...
from core.config import (
    DEBUG_MODE, PAGE_TIMEOUT, ACTION_TIMEOUT, STORAGE_STATE, LOCAL_TIMEZONE,
    AUTO_ENABLED, AUTO_MIN_CONCURRENCY, AUTO_MAX_CONCURRENCY,
    CPU_UPPER_THRESHOLD, CPU_LOWER_THRESHOLD, MEM_UPPER_THRESHOLD,
    COOLDOWN_SECONDS, CHECK_INTERVAL, INITIAL_CONCURRENCY, NUM_FORM_SUBMITTERS,
    BASE_DASHBOARD_URL, STORE_PREFIX_RE, config
)
...
from services.chat_service import post_job_summary, flush_pending_chat_entries
...
async def auto_concurrency_manager(state: ScraperState):
    ...
    while True:
        ...
        if now - state.last_concurrency_change >= COOLDOWN_SECONDS:
            if (cpu > CPU_UPPER_THRESHOLD or mem > MEM_UPPER_THRESHOLD) and state.concurrency_limit > AUTO_MIN_CONCURRENCY:
                state.concurrency_limit -= 1
                state.last_concurrency_change = now
                app_logger.info(f"Auto-concurrency: decreased to {state.concurrency_limit} (CPU {cpu:.1f}%, MEM {mem:.1f}%)")
            elif cpu < CPU_LOWER_THRESHOLD and mem < MEM_UPPER_THRESHOLD and state.concurrency_limit < AUTO_MAX_CONCURRENCY:
                state.concurrency_limit += 1
                state.last_concurrency_change = now
                app_logger.info(f"Auto-concurrency: increased to {state.concurrency_limit} (CPU {cpu:.1f}%, MEM {mem:.1f}%)")
                
            if state.concurrency_limit > AUTO_MAX_CONCURRENCY: state.concurrency_limit = AUTO_MAX_CONCURRENCY
            if state.concurrency_limit < AUTO_MIN_CONCURRENCY: state.concurrency_limit = AUTO_MIN_CONCURRENCY
            
            async with state.concurrency_condition:
                state.concurrency_condition.notify_all()
        await asyncio.sleep(CHECK_INTERVAL)

async def worker_task(worker_id: int, browser: Browser, storage_template: dict, job_queue: asyncio.Queue, submission_queue: asyncio.Queue, state: ScraperState):
...
async def process_urls(browser: Browser, state: ScraperState):
    ...
    app_logger.info(f"Processing finished. Processed {state.progress['current']}/{state.progress['total']} in {elapsed:.2f}s")
